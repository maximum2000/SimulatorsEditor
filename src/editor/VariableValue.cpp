...
namespace ScenariosEditorScenarioElement
{
	...
	VariableValue::VariableValue(std::vector<std::string>* args) : ScenarioElement::ScenarioElement(args)
	{
		...
		variable_A = SetAttributeFromSource(args, 2);
		variable_fix_A = SetAttributeFromSource(args, 2);
		variable_B = SetAttributeFromSource(args, 2);
		variable_fix_B = SetAttributeFromSource(args, 2);
	}
	VariableValue::VariableValue() : ScenarioElement::ScenarioElement()
	{
		variable_X = ElementAttribute(u8"variable_X",0);
		variable_A = ElementAttribute(u8"variable_A",2);
		variable_fix_A = ElementAttribute(u8"variable_fix_A",2);
		variable_B = ElementAttribute(u8"variable_B",2);
		variable_fix_B = ElementAttribute(u8"variable_fix_B",2);
	}
